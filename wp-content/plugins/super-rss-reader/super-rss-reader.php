    ...
    if($ucount > 1){
        ...
        for( $i=0; $i < $ucount; $i++ ){
            ...
            if (!is_wp_error($rss)){
                ...
                echo '<li data-tab="srr-tab-' . $rand[$i] . '" style="text-transform: capitalize; font-size:13px;color:#fffffe;font-weight:500;">' . $rss_title . '</li>';
            }else{
                echo '<li data-tab="srr-tab-' . $rand[$i] . '" style="text-transform: capitalize; font-size:13px;color:#fffffe;font-weight:500;">Error</li>';
            }
    ...
    for($i=0; $i<$ucount; $i++){
        ...
        if ($maxitems == 0){
            ...
            foreach ($rss_items as $item){
                ...
                $pdatezero = $item->get_date('j');
                $pdays = sprintf("%02d", $pdatezero);
                $pmonth = $item->get_date('M');
                ...
                $dstitle = strip_tags($title);
                $newtitle = explode("-", $dstitle);

                $dnwtitle = "";
                for ($dissn=0; $dissn < (count($newtitle) - 1); $dissn++) { 

                    $dnwtitle .= $newtitle[$dissn]." ";
                }
                ...
                echo '<div class="srr-item ' . (($j%2 == 0) ? 'even' : 'odd') . '" style="display: inline-flex;">';
                echo '<div class="srr-item-cont" style="">';
                echo '<time class="srr-date">' . $pdays . '<div>' . $pmonth . '</div></time>';
                echo '</div>';
                echo '<div class="srr-content-cont" style="display: block;">';
                echo '<div class="srr-title '.count($newtitle).'">';
                echo '<a href="' . $link . '"' . $newtab . $nofollow . ' title="' . $dnwtitle . '">';
                echo '<div class="dnewstitle">'. (strlen($dstitle) > 46 ? substr($dstitle, 0, 45) . '...' : $dstitle) .'</div>';
                echo '<div class="dauthor">'. $author .'</div>';
                echo '</a>';
                echo '</div>';
                echo '<div class="srr-meta">';

                
                // (strlen($newtitle) > 46 ? substr($newtitle, 0, 45) . '...' : $newtitle)
                // echo '<img src="https://1948265747.rsc.cdn77.org/svg/trend.svg" width="21px">';
