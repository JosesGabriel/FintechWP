		...
		var el_autocomplete = el.autocomplete({
			...
			source: function( request, response ) {
				if ( um_extractLast( request.term ).charAt(0) === '@' ) {
					jQuery.getJSON( wp.ajax.settings.url + '?action=um_activity_get_user_suggestions&term=' + um_extractLast( request.term ), function( data ) {
						response( data );
					});
				}
			},
		...
		apu.uploadFile({
			...
			onError: function (files,status,errMsg,pd) {
				...
				widget.find('.upload-statusbar').prev('div').append(
					'<div class="um-error-block">' + errMsg + '</div>'
				);
...
jQuery( document ).ready(function () {
	...
	if ( wall_post > 0 ) {
		if ( ! wall_comment ) {
			...
			// 	}
			// });
			var $el = jQuery('#postid-' + parseInt( wall_post ));
			...
			var $el = jQuery('#commentid-' + parseInt( wall_comment ));
	...
	//=========================================================//
	/*
		var user_login = jQuery(this).parents('.um-activity-commentl').find('input.userlogin').attr('value');
									
		console.log(user_login);

		var user_login2 = user_login.split(' ');

	 				if (comment != '') {
			                var newconts = '';

			                var dprocessedtext = comment.split(' ');

			                jQuery.each(dprocessedtext, function(index, item) {
		                	
			              		
			              		if(item.indexOf('@') == false) {

			              			var usname = item.replace('@','');
			              			var infors = usname.split('_');
			              			var uid = infors[0];
			              			
									var newword = "";

									for(var i = 1 ; i < infors.length; i++){

										newword += infors[i] + " ";
									}

									var finalword = '<a href="https://arbitrage.ph/user/'+user_login2[0]+'/" target="_blank" class="user_tag">@'+newword+'</a>';
		...
		if(!jQuery(this).hasClass("isclicked")){
			...
			if (!jQuery(this).parents('.um-activity-widget').hasClass('unready')) {
				...
				if (parseInt(numberbullx) > 0) {
					...
					jQuery(this).parents('.um-activity-foot.status').find('.um-activity-bullish a').removeClass("isclicked");
	...
	// activate un bearish
	jQuery( document.body ).on('click', '.um-activity-left.um-activity-actions > div .dnumof', function(e) {
		e.preventDefault();
		...
		jQuery(this).parents('.um-activity-foot.status').find('.dpartmodal *[data-btname="'+gettab+'"]').addClass('active');
		jQuery(this).parents('.um-activity-foot.status').find('.totab > .'+gettab).show().addClass('active');
		...
		// 	dataType: 'json',
		// 	data: {
		// 		action:'um_get_activity_bearish',
		// 		postid: postid
		// 	},
		// 	success: function (data) {
		// 		console.log(data);
	...
	jQuery( document.body ).on('click', '.dtabspart .tabbutton li', function(e){
		...
		jQuery(this).parents('.modal-content').find('.totab > div').hide().removeClass('active');


		jQuery(this).parents('.modal-content').find('.totab > .'+dtab).show().addClass('active');
	});
	...
	jQuery( document.body ).on('click', '.um-activity-like:not(.active) a', function(e) {
		...
		if (!jQuery(this).parents('.um-activity-widget').hasClass('unready')) {
			...
			jQuery(this).find('i').addClass('um-active-color');
			// var count = jQuery(this).parents('.um-activity-widget').find('.um-activity-post-likes');
			// var count = jQuery(this).find('.dnumof');
			// count.html(parseInt(count.html()) + 1);
			var dlikesnum = jQuery(this).parents('.um-activity-like.notyours').attr('data-numlikes')
			jQuery(this).parents('.um-activity-like.notyours').attr('data-numlikes', parseInt(dlikesnum) + 1);
			jQuery(this).parents('.um-activity-like.notyours').find('.dnumof').text(parseInt(dlikesnum) + 1);
			jQuery(this).parent().addClass('active').addClass('isyours').removeClass('notyours');
			...
			var numbearish = jQuery(this).parents('.um-activity-foot.status').find('.um-activity-bearish.isyours').attr('data-numbear');
			if (parseInt(numbearish) > 0) {
				jQuery(this).parents('.um-activity-foot.status').find('.um-activity-bearish.isyours').attr('data-numbear', parseInt(numbearish) - 1);
				jQuery(this).parents('.um-activity-foot.status').find('.um-activity-bearish.isyours .dnumof').text(parseInt(numbearish) - 1);
	...
	jQuery( document.body ).on('click', '.um-activity-like.active a', function(e) {
		...
		var numlikes = jQuery(this).parents('.um-activity-foot.status').find('.um-activity-like.isyours').attr('data-numlikes');
		if (parseInt(numlikes) > 0) {
			jQuery(this).parents('.um-activity-foot.status').find('.um-activity-like.isyours').attr('data-numlikes', parseInt(numlikes) - 1);
			jQuery(this).parents('.um-activity-foot.status').find('.um-activity-like.isyours .dnumof').text(parseInt(numlikes) - 1);
		...
		// jQuery(this).parents('.um-activity-like').attr('data-numlikes', parseInt(jQuery(this).parents('.um-activity-like').attr('data-numlikes') - 1));
