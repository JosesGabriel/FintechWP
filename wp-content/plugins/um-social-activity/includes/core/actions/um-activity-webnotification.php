...
if ( ! defined( 'ABSPATH' ) ) exit;
	...
	***/
	...
	function um_activity_add_notification_type( $array ) {
		...
		$array['new_post_bull'] = array(
			'title' => __('User get a new post bullish','um-activity'),
			'template' => '<strong>{member}</strong> bullished your wall post.',
			'account_desc' => __('When someone reacts to your post','um-activity'),
		);

		$array['new_post_bear'] = array(
			'title' => __('User get a new post bearish','um-activity'),
			'template' => '<strong>{member}</strong> bearished your wall post.',
			'account_desc' => __('When someone reactis to your post','um-activity'),
		);
	...
	***/
	...
	function um_activity_add_notification_icon( $output, $type ) {
		...
		if ( $type == 'new_post_bull' ) {
			$output = '<i class="um-faicon-thumbs-up" style="color: #1c6dc9"></i>';
		}
		
		if ( $type == 'new_post_bear' ) {
			$output = '<i class="um-faicon-thumbs-up" style="color: #1c6dc9"></i>';
		}
