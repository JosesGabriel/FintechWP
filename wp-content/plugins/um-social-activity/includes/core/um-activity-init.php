...
class UM_Activity_API {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_um_activity_bullish_post', array( $this->api(), 'ajax_bullish_post' ) );
		add_action( 'wp_ajax_um_activity_bearish_post', array( $this->api(), 'ajax_bearish_post' ) );
		
		add_action( 'wp_ajax_um_activity_unbullish_post', array( $this->api(), 'ajax_unbullish_post' ) );
		add_action( 'wp_ajax_um_activity_unbearish_post', array( $this->api(), 'ajax_unbear_post' ) );

		add_action( 'wp_ajax_um_get_activity_bearish', array( $this->api(), 'ajax_get_bearish' ) );
	...
	function download_routing() {
		...
		if ( empty( get_query_var( 'um_post' ) ) ) {
			return false;
		}
		...
		if ( empty( get_query_var( 'um_author' ) ) ) {
			return false;
		}
		...
		if ( empty( get_query_var( 'um_verify' ) ) ||
		     ! wp_verify_nonce( get_query_var( 'um_verify' ), $author_id . $post_id . 'um-download-nonce' ) ) {
			return false;
		}
