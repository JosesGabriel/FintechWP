        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $.each(data.data, function(i, value){
                    ...
                    addliveme += '<td style="width:9%" class="table-cell-live">'+(value.position).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")+'</td>';
                    addliveme += '<td style="width:12%" class="table-cell-live">₱'+(value.aveprice).toFixed(3).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")+'</td>';
