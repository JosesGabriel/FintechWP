...
<script>
    if (typeof angular !== 'undefined') {
		...
		foreach ($havemeta as $key => $value) {    
           ...
           // $charthistory = '/wp-json/data-api/v1/charts/history?symbol=' . $value['stockname'] . '&exchange=PSE&resolution=1D&from='. date('Y-m-d', strtotime("-20 days")) .'&to=' . date('Y-m-d');
            ...
            //$chartintraday = '/wp-json/data-api/v1/charts/history/intraday?symbol=' . $value['stockname'] . '&exchange=PSE';
			...
			if ($currentTime >= $startTime && $currentTime <= $endTime) {
			  	...
			  	$curl = curl_init();
				curl_setopt($curl, CURLOPT_URL, '/wp-json/data-api/v1/charts/history/intraday?symbol=' . $value['stockname'] . '&exchange=PSE' );
		...
		$("#submitmenow").click(function(e){
			...
			if(isstock != ""){
				if(!(eps == "" && tfs == "" && sls == "")){
					...
					$('.addwatchtab .selectnotifitems').prepend('<div class="indicatorerror errorpop">You have add a value to atleast one indicator</div>');
